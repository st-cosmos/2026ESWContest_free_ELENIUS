...
출항 화면에서 얼굴 인식이 성공하면 승선 처리한다.
- 구명조끼 장치가 배정된 선원은 착용 상태여야 승선 처리된다(한국 어선 착용 의무).
- 첫 승선 시 시동 잠금이 해제된다.
- 모든 승선 이력은 boarding_logs.json 에 날짜별로 누적 저장된다.
"""
...
class BoardingManager:
    ...
    def handle_recognition(self, user: dict) -> None:
        ...
        with self._lock:
            if user_id in self._boarded_ids:
                if self._cooldown_ok(f"re:{user_id}"):
                    self._overlay.set(f"이미 승선 완료된 선원입니다 ({name})", COLOR_WARN)
                return
            ...
            first_boarding = len(self._session) == 0
            now = datetime.now()
            entry = {
                "user_id": user_id,
                "name": name,
        ...
        if first_boarding:
            self._engine.unlock()
            self._overlay.set(f"{name} 님 승선 완료 · 시동 잠금 해제", COLOR_OK)
        else:
            suffix = " · 구명조끼 착용 확인" if worn else ""
            self._overlay.set(f"{name} 님 승선 완료{suffix}", COLOR_OK)
    ...
    # ── 누적 로그 ────────────────────────────────────────────────────────
    def logs_grouped(self) -> list[dict]:
        """날짜별(최신순) 승선 로그."""
