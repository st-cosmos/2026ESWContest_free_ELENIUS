...
### BLE 브로드캐스트 (기본, `../firmware` nRF52840 장치)

구명조끼 장치(E73-2G4M08S1C)는 연결 없이 BLE 광고 패킷으로 착용/생존/낙하를
브로드캐스트하고, 서버가 시작 시 자동으로 스캐너(`jacketble.py`, bleak)를 띄워
수신합니다 — 광고 수신 자체가 생존 신호이고, 익수 시 수중 전파 차단으로 신호가
끊기는 것을 기존 타임아웃 로직이 그대로 판정합니다. 광고 페이로드 규격은
[`../firmware/README.md`](../firmware/README.md) 참고.

| 환경 변수 | 기본값 | 설명 |
|---|---|---|
| `VPASS_BLE` | `auto` | `off` 면 BLE 스캔 비활성 (bleak 미설치·어댑터 없음 시 자동 비활성) |
| `VPASS_BLE_COMPANY_ID` | `0xFFFF` | 광고 Manufacturer Data 회사 ID |
| `VPASS_JACKET_BATT_WARN_MV` | `2200` | 배터리 교체요망 임계 전압(mV). 이 아래(또는 장치 저전압 플래그)로 착용이 감지되면 경고 모달 표시 |

장치 배터리는 광고 패킷의 전압 원값으로 구명조끼 모니터에 표시되고(AA 리튬은
방전 곡선이 평평해 % 환산이 부정확), 교체요망 상태로 **착용이 감지되면** 경고
모달이 뜹니다 — 해당 장치를 탈의(배터리 교체)하면 자동으로 사라집니다.
시뮬레이터의 `배터리 부족`/`배터리 정상` 버튼으로 하드웨어 없이 시연할 수 있습니다.

### HTTP (레거시 ESP8266 / 외부 연동용)

구 펌웨어(`smart-vpass-example/life-jacket`)와의 호환 경로도 그대로 동작합니다.

```
POST /api/wearing {"device": "jacket-1", "worn": true}    # 착용/해제
