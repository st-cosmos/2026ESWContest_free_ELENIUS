...
4. **탈의/보관** — 버클을 풀면 worn=0 을 60초 광고한 뒤 System OFF 로 잠든다
   ...
   `CONFIG_JACKET_STORAGE_SLEEP=n` 이면 System OFF 를 아예 생략하고 깨어
   있는 채 광고만 멈춘다 (버클 재착용 시 즉시 재개). 잠들면 BLE/USB 가
   전부 꺼져 무반응이 되고 SENSE 웨이크업 실패 사례가 있어(2026-08-29)
   현재 `prj.conf` 는 이 옵션을 끈 시연 구성이다.
